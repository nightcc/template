...
def get_parser():
    ...
    parser.add_argument('--epochs', metavar='EPOCH', type=int, default=2,  help='number of total epochs to run (default: 2)')
    parser.add_argument('--batch_size', '-b', metavar='BATCH_SIZE', type=int, default=256, help='mini-batch size (default: 256)')
    ...
    parser.add_argument('--extras', default=None, type=str, help='file with extra configuration information')
    ...
    optimizer_args.add_argument('--weight_decay', '--wd', metavar='W', type=float, default=1e-4, help='weight decay (default: 1e-4)')
    ...
    load_checkpoint_group_exc.add_argument('--resume', dest='deprecated_resume', action='store_true', help='resume to train model')
    load_checkpoint_group_exc.add_argument('--resume-from', dest='resumed_checkpoint_path', default='', type=str, metavar='PATH',
...
def float_range(min_val=0., max_val=1., exc_min=False, exc_max=False):
    def checker(val_str):
        ...
        if min_op(val, min_val) and max_op(val, max_val):
            ...
            raise ValueError('Value must be {} {} and {} {} (received {})'.foramt(min_op_str, min_val, max_op_str, max_val, val))
